...
public class QLearning
{
	...
	*/
	public void incrementarEpsilon(double incremento)
	{
		if(this._epsilon < 0.95)
		{
			...
			if(this._epsilon > 0.95) this._epsilon = 0.95;
		}
	}
	...
	*/
    ...
    /**
     * Carga la tabla Q contenida en un fichero pasado como parametro.
     */
    public void cargarTablaQ(String nombreTablaQ){
		try{
			BufferedReader br = new BufferedReader(new FileReader (new File (nombreTablaQ)));
			String linea = "";
			int lineas = 0;
			while((linea=br.readLine())!=null){
				...
				while(i < numTokens){
					...
					this._tablaQ[lineas][i] = Double.parseDouble(elemento);
					i++;
				}
    ...
    /*
     * Guarda la tabla Q en el fichero especificado como parametro
     */
    public void guardarTablaQ(String nombreTablaQ){
        try{
	        BufferedWriter fichero = new BufferedWriter ( new FileWriter(nombreTablaQ, false) );
			for(int i = 0; i < this._tablaQ.length; i++){
				for(int j = 0; j < this._tablaQ[0].length; j++){
					fichero.write(this._tablaQ[i][j] + " ");
				}
				fichero.write("\n");
			}
			fichero.close();
	...
	/*
	* Muestra el contenido de la tabla Q por pantalla
	*/
